...
// Mock restaurant data (replace with Yelp API later)
...
export default function FoodDateScreen({ route, navigation }) {
  ...
  const [selectedTime, setSelectedTime] = useState(new Date(Date.now() + 19 * 60 * 60 * 1000)); // 7 PM today
  ...
  const sendFoodDateProposal = async () => {
    console.log('Send button pressed!'); // Debug log
    ...
    console.log('Selected restaurant:', selectedRestaurant); // Debug log
    console.log('Match details:', { matchId, partnerId, partnerName }); // Debug log
    ...
    try {
      ...
      proposedDateTime.setHours(selectedTime.getHours());
      proposedDateTime.setMinutes(selectedTime.getMinutes());
      ...
      const { data: proposal, error } = await supabase
        ...
        .insert({
          match_id: matchId,
          proposed_by: user.id,
          proposed_to: partnerId,
          restaurant_name: selectedRestaurant.name,
          restaurant_address: selectedRestaurant.address,
          restaurant_cuisine: selectedRestaurant.cuisine,
          restaurant_rating: selectedRestaurant.rating,
          restaurant_price_level: selectedRestaurant.priceLevel,
      ...
      if (error) {
        ...
        if (error.message && error.message.includes('relation "food_date_proposals" does not exist')) {
          throw new Error('Food date system not set up yet. Please run the SQL setup script in your Supabase dashboard first.');
        }
        throw error;
      }

      // Send cute notification (optional - don't block if it fails)
      try {
        await scheduleCustomNotification({
          title: '🍽️ Dinner Date Invitation!',
          body: `${user.user_metadata?.name || 'Someone'} wants to share ${selectedRestaurant.cuisine} with you at ${selectedRestaurant.name} on ${formatDate(selectedDate)}! 💕`,
          data: {
            type: 'food_date_proposal',
            proposalId: proposal.id,
            matchId: matchId,
            partnerName: user.user_metadata?.name || 'Someone',
          },
        });
        console.log('Notification sent successfully');
      } catch (notificationError) {
        console.log('Notification failed (but continuing):', notificationError.message);
      ...
      Alert.alert('Error', error.message || 'Failed to send date invitation');
    } finally {
      setLoading(false);
    }
  };
